...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec  9 23:33:58 2017
...
train = pd.read_csv('../01_Data/train.csv', nrows=MAX_TRAINING)
train = pd.concat((train, pd.read_csv('../01_Data/data 2/churn_comp_refresh/train_v2.csv')), axis=0, ignore_index=True).reset_index(drop=True)
...
transactions = pd.read_csv('../01_Data/transactions.csv', nrows=MAX_TRANSACTIONS)
transactions = pd.concat((transactions, pd.read_csv('../01_Data/data 3/churn_comp_refresh/transactions_v2.csv')), axis=0, ignore_index=True).reset_index(drop=True)
...
user_logs = pd.read_csv('../01_Data/user_logs.csv', usecols=['msno'], nrows=MAX_LOGS)
user_logs = pd.concat((user_logs, pd.read_csv('../01_Data/user_logs.csv', usecols=['msno'], nrows=MAX_LOGS)), axis=0, ignore_index=True).reset_index(drop=True)
user_logs = pd.DataFrame(user_logs['msno'].value_counts().reset_index())
user_logs.columns = ['msno','logs_count']
#user_logs = []; 

# adds the data of the members. Maps the genders to numbers.
members = pd.read_csv('../01_Data/members_v3.csv', nrows=MAX_MEMBERS)
